...
sub test_insert_select {
	...
	my $has_ins = $track_1->insert;
	is($has_ins, 1);

	my $last_id = $track_1->id;

	my $selected_track = Local::MusicLib::Track->select_by_id($last_id);
	is($selected_track->id, $track_1->id, 'eq id');
	is($selected_track->album_id, $track_1->album_id, 'eq album_id');
	is($selected_track->name, $track_1->name, 'eq name');
	is($selected_track->extension, $track_1->extension, 'eq extension');
	is($selected_track->create_time, $track_1->create_time, 'eq create_time');

	my $has_del = $track_1->delete;
}
